...
* 라이브러리는 편리하지만 충돌이 생길 위험이 크다.
- AOS 라이브러리를 사용했는데 모바일 화면을 넘어서는 경우가 생겨서 쓸모없는 좌우 스크롤이 생겼었다. 이를 몰라서 해결하는데 상당한 시간이 걸렸는데 다음에 라이브러리를 사용하게 되면 이를 참고해서 사용하면 좋을것 같다.
...
<img src="images/mobile-width-issue.gif" width="300">

* AOS 라이브러리 충돌로 인해 좌우 스크롤이 되었다. 기존의 AOS 라이브러리의 경우 좌,우에서 등장했는데 width를 벗어나지 않게 상,하에서 등장하게 했더니 문제가 해결이 되었다.
